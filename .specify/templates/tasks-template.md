...
**Tests**: Respect constitution TDD mode. If TDD is Active, test tasks are
mandatory and MUST be written/failing before implementation. If TDD is Deferred,
...
- [ ] T002 Initialize Python + React project structure with local-only runtime assumptions
...
- [ ] T010 Confirm mock-stage security scope (no production secrets/PII) and list deferred hardening tasks
...
### Tests for User Story 1 (depends on TDD mode) ⚠️

> **NOTE**: If TDD is Active, write these tests FIRST and ensure they FAIL
> before implementation. If TDD is Deferred, include only when requested.
...
- [ ] T019 [US1] If mobile UX is impacted, document Expo validation evidence
