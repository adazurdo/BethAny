...
**Amendment 4 (superseded by `006-elo`, 2026-07-17)**: La spec `006-elo` introdujo un saldo de coins y una liquidación real de las apuestas de partido, lo que **sustituye** las siguientes afirmaciones de esta spec: el importe del boleto ya **no** es puramente ilustrativo (Clarifications párrafo 3; Constitution Alignment "Simplicity Statement" y "Security Scope"; Assumptions párrafos 2 y 4), y las apuestas realizadas ya **no** se quedan fijas en `"realizada"` para siempre. Desde `006-elo`:
- Colocar una apuesta (simple o combinada) descuenta su importe del saldo de coins de la cuenta, y se rechaza si el saldo no alcanza (`insufficient coins balance`).
- Cada apuesta se liquida sola un tiempo fijo después de colocarse, contra un resultado de partido simulado de forma determinista (no existe ningún resultado real de partido en el sistema — ver `specs/006-elo/spec.md` Clarifications y `research.md` Decisión 8), pasando su estado de `"realizada"` a `"ganada"` (con crédito de la ganancia potencial al saldo) o `"perdida"`.
- Todo lo demás de esta spec (mercado 1X2, pestañas Simple/Combinada, cuota combinada como suma, animaciones, ausencia de pestaña "Sistema") sigue vigente sin cambios; ver `specs/006-elo/` para el diseño completo del saldo, la renta periódica y los hitos de ELO, que son ortogonales a esta spec.
