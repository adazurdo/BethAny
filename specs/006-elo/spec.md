...
- **Bloqueo técnico descubierto durante la investigación**: hoy no existe ningún resultado de partido real en el sistema — el sincronizado con football-data.org descarta explícitamente los partidos ya finalizados y no guarda marcador alguno, y las cuotas (`odds.py`) ya son un cálculo determinista sin datos externos. Para poder implementar la liquidación de las apuestas de partido (`PlacedBet`) contra el saldo de Beths sin depender de datos que no existen, esta spec **simula el resultado de cada partido de forma determinista** (mismo patrón ya usado para las cuotas: una función pura sembrada por `match_id`), en vez de esperar o inventar una integración con resultados reales. Esta decisión fue confirmada explícitamente por el usuario.
  - **Superada por la Revisión 2026-07-31**: la app ya integra football-data.org y PandaScore como fuentes reales (`003-datos-mock`), así que la liquidación pasa a consultar el resultado real de cada partido en su propia fuente en vez de simularlo — ver `research.md`, Revisión 2026-07-31, y Decisions 8/9 (marcadas como retirada/revisada). El resto del diseño de esta spec (fórmula de Elo, Beths, hitos, tope diario) no cambia.
...
- **FR-008**: *(Revisado 2026-07-31)* El sistema MUST determinar el resultado real (local/empate/visitante) de cada partido sobre el que existan apuestas colocadas, consultándolo en la misma fuente de datos real que originó ese partido (football-data.org o PandaScore), y MUST tratar un partido cuya fuente aún no lo reporte como terminado como "resultado no disponible todavía", sin inventar ni simular un resultado en su lugar. *(Versión original, retirada: determinar de forma determinista y reproducible a partir del identificador del partido un resultado simulado — ver `research.md`, Decision 8.)*
- **FR-009**: *(Revisado 2026-07-31)* El sistema MUST liquidar cada apuesta pendiente cuyo partido ya se considere suficientemente antiguo (tiempo configurado desde el kickoff real) **y** cuyo resultado real ya esté disponible (FR-008) para todas sus selecciones: acreditando la ganancia potencial al saldo de Beths y marcándola como ganada si el resultado real coincide con la selección (o con todas las selecciones, si es una combinada), o marcándola como perdida sin acreditar nada en caso contrario. Una apuesta cuyo resultado real todavía no esté disponible MUST permanecer pendiente en vez de liquidarse.
- **FR-019**: *(Nuevo 2026-07-31)* El sistema MUST exponer, para cada apuesta ya liquidada, el cambio de Elo exacto que esa apuesta aplicó a la cuenta (`eloDelta`), incluyendo el caso en que no tuvo efecto en Elo por haberse alcanzado el tope diario (FR-003b); el historial de apuestas del usuario (`bets/index.tsx`) MUST mostrar ese valor junto a cada apuesta liquidada, distinguiendo visualmente una ganancia de Elo de una pérdida.
...
- **Apuesta de partido (existente)**: gana un estado de liquidación (pendiente/ganada/perdida), la fecha en la que se liquidó, y (2026-07-31) el Elo exacto (`eloDelta`) que aplicó a la cuenta al liquidarse (nulo mientras está pendiente o si no tuvo efecto por el tope diario); su liquidación es ahora también el único disparador del recálculo de ELO.
...
- **SC-002**: El 100% de las apuestas de partido colocadas terminan en un estado final (ganada o perdida) sin intervención manual, tan pronto como su partido tiene un resultado real confirmado por su fuente *(2026-07-31: ya no garantizado "dentro del tiempo de liquidación configurado" — ese tiempo es ahora solo el punto a partir del cual se empieza a comprobar la fuente real, no una garantía de resultado; ver `research.md`)*.
...
- ~~El resultado simulado de partido introducido por esta spec (ver Clarifications) es una simplificación explícita del prototipo mientras no exista una fuente real de resultados finales; puede sustituirse por datos reales en una spec futura sin cambiar el resto de este diseño (el saldo de Beths y su liquidación no dependen de cómo se obtenga el resultado, solo de que exista uno).~~ **Cumplido el 2026-07-31**: la liquidación ya consulta el resultado real de cada partido en su fuente (football-data.org/PandaScore); el resto del diseño (Beths, hitos, tope diario) no cambió, tal y como se anticipaba aquí.
