...
- El resultado real del partido para liquidar un reto reutiliza exactamente la misma resolución de resultados ya introducida en `006-elo` (`match_results.resolve_match_result`, revisada 2026-07-31 para consultar la fuente real en vez de simular — ver `006-elo/research.md`), la misma ventana de liquidación (`SETTLEMENT_DELAY_MINUTES`, 90 minutos, ahora un throttle previo a la comprobación real) y el mismo patrón de liquidación perezosa (`_settle_due_bets`/`_settle_due_challenges`) — no se introduce un scheduler ni una fuente de resultados nueva.
