...
def list_unseen_elo_milestones(account_id: str) -> list[dict]:
    with get_connection() as connection:
        rows = connection.execute(
            """
            SELECT m.* FROM elo_milestone_awards m
            WHERE m.account_id = ?
              AND NOT EXISTS (
                SELECT 1 FROM notification_seen s
                WHERE s.account_id = m.account_id AND s.mark_key = 'elo_milestone:' || m.tier
              )
...
def ack_elo_milestones(account_id: str) -> None:
    for milestone in list_unseen_elo_milestones(account_id):
        mark_seen(account_id, f"elo_milestone:{milestone['tier']}")
        ...
        (
            {
                ...
                "correctCount": correct_counts[member["accountId"]],
                "elo": member["elo"],
            }
