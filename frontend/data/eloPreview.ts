...
// The Elo gained on a win only ever spans stakeMultiplier's fixed [STAKE_MULT_MIN,
// STAKE_MULT_MAX] range (see stakeMultiplier above), linear in stake and reaching the ceiling
// exactly at MAX_ELO_STAKE. So for a given bet (fixed odds/k), the achievable Elo-on-win is
// bounded regardless of how much the player could stake - `maxBeths` only matters insofar as
// it may cap the stake *below* the point where the ceiling is reached.
...
export function stakeForTargetElo(gamesPlayed: number, odds: number, targetEloGain: number, maxBeths: number): StakeForTarget | null {
  ...
  const rawStake = ((targetMultiplier - STAKE_MULT_MIN) / (STAKE_MULT_MAX - STAKE_MULT_MIN)) * MAX_ELO_STAKE;
